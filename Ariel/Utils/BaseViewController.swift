...
class BaseViewController: UIViewController, BaseViewControllerMethods {
    ...
    func showMessage(_ message: String, okAction: (() -> Void)?, cancelAction: (() -> Void)?) {
        ...
        let okActionBtn = UIAlertAction(title: "ok localized", style: .default, handler: {_ in
            guard let okAction = okAction else { return}
            okAction()
        })
        if let cancelAction = cancelAction {
            let cancelActionBtn = UIAlertAction(title: "cancel localized", style: .default, handler: {_ in
                cancelAction()
            })
